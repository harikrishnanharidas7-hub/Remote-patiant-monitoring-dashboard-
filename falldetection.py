...
app = Flask(__name__)
# Get secret key from environment variable for better security
...
# We will still use the file path name for consistency, but prioritize ENV var/DB
...
# --- Firebase Initialization (SECURELY MODIFIED) ---
FIREBASE_ENV_VAR = "FIREBASE_CREDENTIALS_JSON"
db = None

def initialize_firebase_securely():
    """Initializes Firebase Admin SDK using an environment variable or a local file."""
    global db
    print("Attempting to initialize Firebase...")
    
    # 1. Try to load credentials from the environment variable (BEST PRACTICE for Render)
    credentials_json_string = os.environ.get(FIREBASE_ENV_VAR)
    ...
    if credentials_json_string:
        try:
            ...
            db = firestore.client()
            print("✅ Firebase initialized securely using Environment Variable.")
            return True
        except Exception as e:
            print(f"❌ ERROR initializing Firebase with environment variable: {e}")
            return False
            
    # 2. Fallback to local file (Only for local testing/development)
    else:
        try:
            cred = credentials.Certificate("fire_base_key.json")
            firebase_admin.initialize_app(cred)
            db = firestore.client()
            print("⚠️ Firebase initialized using local file. Set the ENV Var on Render.")
            return True
        except FileNotFoundError:
            print(f"❌ FATAL ERROR: Neither ENV var '{FIREBASE_ENV_VAR}' nor local file 'fire_base_key.json' found.")
            return False
        except Exception as e:
            print(f"❌ ERROR initializing Firebase with local file: {e}")
            return False

# Initialize Firebase before the app starts
if not firebase_admin._apps:
    initialize_firebase_securely()

# Ensure we have a database client if initialization failed for a clean exit
if db is None:
    print("FATAL: Firebase was not initialized. Exiting process.")
    # In a real deployment, you might raise an exception here to crash and restart the service
    # For now, we continue, but any DB call will fail.

# =========================================================
# EMAIL & FALL LOGIC (Unchanged)
# =========================================================
...
# =========================================================
# 🔄 FITBIT ENGINE (Unchanged, relies on DB)
# =========================================================
# NOTE: The Fitbit token handling is already robust as it uses Firestore for persistence,
# which is perfect for deployment environments where local files may be ephemeral.
...
def get_today_activity():
    if not db: return # Skip if Firebase failed to init
...
def login():
    if not db: return jsonify({"error": "Service not available"}), 503
...
def get_dashboard_data():
    ...
    # Sort data just in case Firestore fetch order is slightly off
    all_data.sort(key=lambda x: x.get('date', '')) 

    labels = [d.get('date') for d in all_data]
    steps_data = [d.get('steps') for d in all_data]
    
    weekly_raw = all_data[-7:]
    weekly_labels = [d.get('date') for d in weekly_raw]
    weekly_data = [d.get('steps') for d in weekly_raw]
...
def record_acceleration():
    data = request.json
    x = data.get("x", 0)
    # The check_fall_logic handles conversion to int/float if needed, 
    # but the API contract should ensure the data type is consistent.
    try:
        check_fall_logic(float(x)) 
    except ValueError:
        print(f"Error: Non-numeric value received for x-axis: {x}")
        return jsonify({"status": "error", "message": "Invalid data type for acceleration"}), 400
...
if __name__ == "__main__":
    if CLIENT_ID == "YOUR_CLIENT_ID_HERE" or CLIENT_ID == "23TLXD"":
        print("❌ WARNING: Please update your Fitbit Client ID, or set FITBIT_CLIENT_ID in environment.")

    # Start the background worker only if Firebase initialized successfully
    if db:
        ...
        print("Application stopped due to Firebase initialization failure.")
