...
import ssl # Needed for secure SMTP connection
...
# FIREBASE INIT (Robust initialization for Render deployment)
if not firebase_admin._apps:
    try:
        # 1. PRIORITY: Load credentials from environment variable (Best for Render/Cloud)
        ...
        # 2. FALLBACK: Load from local file (Only for local development)
        elif os.path.exists("fire_base_key.json"):
            cred = credentials.Certificate("fire_base_key.json")
            firebase_admin.initialize_app(cred)
            print("✅ Firebase initialized from local file (Development Mode).")
        
        else:
            print("❌ FIREBASE_CREDENTIALS environment variable and local file 'fire_base_key.json' not found. Database features will be disabled.")
...
# FITBIT CONFIG (Load from environment variables)
...
EMAIL_PASSWORD = os.environ.get("EMAIL_PASSWORD", "rufe akup sfvf vspe ")
...
def send_email_blocking(subject, body_content):
    ...
    if not EMAIL_SENDER or not EMAIL_PASSWORD or not EMAIL_RECEIVER:
        print("❌ Email Failed: Credentials missing.")
        return False
    ...
    try:
        ...
        # CRITICAL: Use SMTP_SSL on port 465
        ...
        # --- AUTHENTICATION FAILURE LOG (MOST COMMON ISSUE) ---
        print("❌ Email Failed: SMTP Authentication Error (Check 16-char App Password and Sender Email Environment Variables).")
        return False
    except smtplib.SMTPServerDisconnected:
        # --- CONNECTION FAILURE LOG ---
        print("❌ Email Failed: SMTPServerDisconnected (Check Port 465 and general network firewall).")
        return False
    except Exception as e:
        # --- GENERIC FAILURE LOG ---
...
# =========================================================
# 🔄 FITBIT ENGINE (Background worker - Safe to block)
# =========================================================
...
def get_today_activity():
    if not db: return # Skip if DB failed initialization
    ...
    if steps >= DAILY_GOAL and not email_sent:
        ...
        # Blocking email send is OK here as this is a background worker thread.
...
def get_dashboard_data():
    ...
    steps_data = [d.get('steps', 0) for d in all_data] # Use .get for safety
    
    weekly_raw = all_data[-7:]
    weekly_labels = [d['date'] for d in weekly_raw]
    weekly_data = [d.get('steps', 0) for d in weekly_raw]
...
def dashboard():
    ...
    if not db:
        flash("CRITICAL ERROR: Database connection failed. Check logs.", "error")
...
def record_acceleration():
    """
    This endpoint is fully non-blocking and responds quickly.
    I/O operations (like email) are handled by a background thread.
    """
    try:
        data = request.json
        ...
        print(f"Error in record_acceleration: {e}")
